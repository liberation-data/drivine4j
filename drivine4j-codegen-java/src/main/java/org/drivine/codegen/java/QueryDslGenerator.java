...
public class QueryDslGenerator {
    ...
    private final List<TypeElement> fragmentClasses;
    ...
    public QueryDslGenerator(
        ...
        this(elementUtils, typeUtils, filer, messager, graphViewClasses, java.util.Collections.emptyList());
    }
    ...
    public QueryDslGenerator(
            ...
            Messager messager,
            List<TypeElement> graphViewClasses,
            List<TypeElement> fragmentClasses) {
    ...
    public void generateAll() {
        // Standalone fragment DSLs — a <F>QueryDsl (NodeReference at alias "n" + static INSTANCE), so a
        // bare @NodeFragment is queryable via query(F.class).filterWith(FQueryDsl.class). No overload
        // ambiguity concern here (Java has no extension functions), so one is generated per fragment.
        for (TypeElement fragmentClass : fragmentClasses) {
            generateFragmentDsl(fragmentClass);
        }
    ...
    /**
     * Adds a property-reference getter for each fragment field. [aliasExpr] is the Java expression the
     * emitted `new PropertyReference(<aliasExpr>, name)` uses for the node alias — the `alias` field
     * for a Properties class, or the literal `"n"` for a standalone fragment QueryDsl. Honors
    ...
    /**
     * Generates a standalone `<Fragment>QueryDsl` for a bare `@NodeFragment`: a
     * {@link org.drivine.query.dsl.NodeReference} at the fragment root alias `"n"` exposing the node's
     * property getters (+ `instanceOf()` for sealed/abstract fragments), with a static `INSTANCE`. Used
     * from Java via {@code gom.query(F.class).filterWith(FQueryDsl.class).where(q -> q.prop().eq(x))}.
     */
    private void generateFragmentDsl(TypeElement fragmentClass) {
        String simpleName = fragmentClass.getSimpleName().toString();
        ...
        try {
            JavaFile javaFile = JavaFile.builder(packageName, classBuilder.build())
                ...
                .build();
            javaFile.writeTo(filer);
            messager.printMessage(Diagnostic.Kind.NOTE, "Generated " + dslClassName + " in " + packageName);
        } catch (IOException e) {
            messager.printMessage(Diagnostic.Kind.ERROR,
