...
/**
 * Generates the Cypher load query for a single @GraphView: the MATCH, the WITH projection of the
 * root fragment plus every relationship (pattern comprehensions, nested views, recursive
 * expansion, relationship fragments, collection sorts), and the RETURN map.
 *
 * Supports recursive (self-referential) relationships and chain cycle detection. Self-referential
 * relationships generate nested pattern comprehensions to a configurable depth. Chain cycles
 * (A → B → C → A) are detected via a visit counter and terminated based on the closing
 * relationship's maxDepth.
 *
 * One instance is created per build (see [GraphViewQueryBuilder]); all per-build state lives in the
 * injected [BuildContext], so this class holds no mutable query state of its own.
 */
